export default {
  ...
  group: {
    ...
    usage: {
      ...
      loading: '正在加载模型用量…',
      loadFailed: '无法加载模型用量。',
      stale: '模型用量可能已过期；后台刷新失败。',
      refresh: '刷新用量',
      retry: '重新加载',
      emptyTitle: '暂无模型用量',
      emptyDescription: '当前分组在统计区间内没有可展示的（凭据, 模型）组合。',
      emptyFilterTitle: '没有符合条件的组合',
      emptyFilterDescription: '关闭「只看冷却中」后再试。',
      onlyCooling: '只看冷却中',
      groupByModel: '按模型折叠',
      groupCount: '共 {count} 项',
      caption: '此分组的凭据 × 模型用量矩阵',
      scrollHint: '左右滚动查看全部列',
      columns: {
        credential: '凭据',
        model: '上游模型',
        tokens: '本窗口 token',
      ...
      cooling: '冷却中',
      cooldownUntil: '{time} 恢复（{remaining}）',
      none: '—',
    },
  },
} as const
