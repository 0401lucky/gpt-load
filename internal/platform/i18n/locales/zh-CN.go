package locales

// MessagesZhCN contains Simplified Chinese control-plane translations.
var MessagesZhCN = map[string]string{
	"donation.not_found":                         "捐献批次不存在",
	"donation.target_changed":                    "目标分组配置已变化，请刷新分组选择。",
	"donation.target_unavailable":                "目标分组当前无法接收捐献。",
	"donation.unavailable":                       "捐献集成当前不可用。",
	"donation.item_not_found":                    "捐献条目不存在",
	"donation.review_not_found":                  "审核动作不存在",
	"donation.review_conflict":                   "审核动作与当前条目状态冲突",
	"donation.review_unavailable":                "该捐献条目当前无法审核",
	"donation.test_unavailable":                  "该捐献条目当前无法进行真实调用测试",
	"donation.test_conflict":                     "测试请求与其已保存的身份不一致",
	"donation.test_busy":                         "该捐献条目已有测试正在进行",
	"donation.test_not_found":                    "测试记录不存在",
	"access_key.custom_invalid":                  "密钥最多 256 个字符，仅支持英文字母、数字和英文符号，不允许空白。",
	"access_key.admin_conflict":                  "不能使用管理员密钥作为访问密钥。",
	"access_key.exists":                          "此访问密钥已存在，请使用其他密钥。",
	"common.success":                             "操作成功",
	"route.not_found":                            "路由不存在",
	"route.method_not_allowed":                   "请求方法不被允许",
	"bad_request":                                "请求错误",
	"request_too_large":                          "请求体过大",
	"bad_gateway":                                "上游服务错误",
	"internal_error":                             "内部错误",
	"idempotency.required":                       "必须提供 Idempotency-Key",
	"idempotency.invalid":                        "Idempotency-Key 必须是规范 UUID v4",
	"idempotency.reused":                         "Idempotency-Key 已用于其它请求",
	"idempotency.expired":                        "幂等结果保留期已过",
	"control.operation_incomplete":               "资源已提交，但运行时恢复尚未完成",
	"control.recovery_pending":                   "较早提交的操作仍在恢复",
	"reset_credit.unavailable":                   "当前没有可用的重置卡",
	"reset_credit.rejected":                      "上游未能使用这张重置卡",
	"reset_credit.outcome_unknown":               "无法确认重置结果；请使用同一幂等键重试，系统不会使用新键重复消耗",
	"model_price.not_found":                      "模型价格不存在",
	"model_price.unpriced_confirmation_required": "将模型价格标记为未定价需要明确确认",
	"model_price.referenced":                     "模型价格仍被分组引用",
	"model_price.automatic_delete_forbidden":     "自动模型价格不能手动删除",
	"auth.invalid_key":                           "无效的授权密钥",
	"auth.forbidden":                             "当前密钥无权访问此功能",
	"auth.locked":                                "认证尝试过多，请稍后重试",
	"group.not_found":                            "分组不存在",
	"group.name_exists":                          "分组名称已存在",
	"group.in_use":                               "分组仍被访问密钥引用",
	"group.channel_target_conflict":              "已有分组使用相同渠道目标",
	"group.model_name_conflict":                  "分组内客户端模型名称必须唯一",
	"group.no_active_credential":                 "该分组没有可用凭据",
	"credential.not_found":                       "凭据不存在",
}
