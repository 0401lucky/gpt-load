...
// autoDecisionAttributionMigrationIndex 返回 0022 在注册表中的下标。
// 上游 0019_auto_decision_attribution 在本 fork 中顺延为 0022，因此不能沿用上游的固定下标。
func autoDecisionAttributionMigrationIndex(t *testing.T) int {
	t.Helper()
	for index, entry := range migrations {
		if entry.ID == migrationfiles.ID0022 {
			return index
		}
	}
	t.Fatal("migration 0022 is not registered")
	return -1
}

func TestAutoDecisionAttributionMigrationCleansInterruptedSwapBackup(t *testing.T) {
	db := openInternalMigrationTestDatabase(t)
	index := autoDecisionAttributionMigrationIndex(t)
	if err := applyMigrationRegistry(db, migrations[:index]); err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&schemaMigration{ID: migrationResumeMarker(migrations[index].ID)}).Error; err != nil {
		t.Fatal(err)
	}
