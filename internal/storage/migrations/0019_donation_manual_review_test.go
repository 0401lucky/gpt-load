...
func TestDonationManualReviewMigrationUpgradesLegacyHistory(t *testing.T) {
	...
	if err := db.AutoMigrate(migrations.SchemaModels0018()...); err != nil {
		t.Fatalf("create the 0018 donation schema: %v", err)
	}
	if err := migrations.Validate0018(db); err != nil {
		t.Fatalf("Validate0018() error = %v", err)
	}
	const (
	...
	if err := migrations.Validate0019(db); err != nil {
		t.Fatalf("Validate0019() error = %v", err)
	}
	...
	if err := migrations.Up0019(db); err != nil {
		t.Fatalf("second Up0019() error = %v", err)
	}
	if err := migrations.Validate0019(db); err != nil {
		t.Fatalf("second Validate0019() error = %v", err)
	}
	if err := migrations.Validate0018(db); err != nil {
		t.Fatalf("the frozen 0018 validator no longer passes after the upgrade: %v", err)
	}
