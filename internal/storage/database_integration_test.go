...
func TestExternalDatabaseLifecycle(t *testing.T) {
	...
	for table, columns := range map[string][]string{
		...
		"credentials": {"identity_fingerprint", "secret_version", "auth_state", "auth_error_code", "proxy_config", "note"},
	...
	if len(migrationIDs) != 24 || migrationIDs[0] != "0001_initial" ||
		...
		migrationIDs[8] != "0009_price_multipliers" || migrationIDs[9] != "0010_model_cooldown" || migrationIDs[10] != "0011_custom_access_keys" || migrationIDs[11] != "0012_access_key_mask_prefix" || migrationIDs[12] != "0013_validation_protocol" || migrationIDs[13] != "0014_affinity_kind" || migrationIDs[14] != "0015_group_usage_index" || migrationIDs[15] != "0016_credential_quota_history" || migrationIDs[16] != "0017_request_log_operation_index" || migrationIDs[17] != "0018_donation_intake" || migrationIDs[18] != "0019_donation_manual_review" || migrationIDs[19] != "0020_credential_note" || migrationIDs[20] != "0021_auto_model" || migrationIDs[21] != "0022_auto_decision_attribution" || migrationIDs[22] != "0023_client_model_overrides" || migrationIDs[23] != "0024_request_audit" {
		t.Fatalf("migration ledger = %v, want complete 0001-0024 chain", migrationIDs)
	}
