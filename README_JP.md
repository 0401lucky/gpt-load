...
バッチは `validation_mode: "manual_review"` を付けて送信することもできます。この場合、機能確認は `features: ["manual_review_v1"]` と独立した `review_limits` を返し、各項目は暗号化して保存されるだけで `pending_review` に留まります。自動検証の経路には入らず、リソースを先行して占有せず、単独で報酬を付与することもありません。手動モードのリクエストはグループの `manual_target_revision` を凍結する必要があります。これは実際の実行構成に対して別途計算される HMAC で、グループ名の変更では無効になりません。このフィールドを省略する従来の呼び出し元は元の `auto` 動作を保ち、リクエスト比較用ダイジェストはバイト単位で変わりません。

項目の待機中、管理者は `GET /batches/:batch_id/items/:item_id/review-context` を取得し、`POST /batches/:batch_id/items/:item_id/tests` で**その項目自身の保存済みキー**に対して実際のチャット呼び出しを 1 回実行し、`POST /batches/:batch_id/items/:item_id/review-actions`（`enter_review`、`approve`、`reject`）で判断できます。各操作は呼び出し元が指定する `action_id` を持ち、それが `Idempotency-Key` を兼ねるため、応答が失われても保存済みコマンドと照合され二重実行されません。テストはその項目のキーのみを使用し、他の認証情報やグループへ切り替わらず、通常の Relay を通らず、いかなるウォレットにも触れません。認証情報の正式登録とランタイム公開まで完了した承認だけが `accepted` を生み、テスト結果そのものが自動承認や自動拒否を行うことはありません。

`approve` と `reject` は実行中のテストと排他的で、保存期限を過ぎた項目は承認できず、拒否にグループのオンライン状態は不要です。自動検証を使い切ったが保存材料が残る項目は `enter_review` で明示的に手動審査へ移せます。元のバッチ、リクエスト摘要、自動試行回数、寄付者、期限はすべて保持されます。保存期間は延長されず、移行・テスト・再試行はいずれも元の 7 日期限を守ります。

実装は `internal/control/donation_*.go` と `internal/storage/migrations/0016_donation_manual_review.go` を参照してください。
